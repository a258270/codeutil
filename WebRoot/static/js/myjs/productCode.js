 ...
 */	
	...
	function saveD(){
		...
		var isBox = $("#isBox").val();			 //是否选项
		var isList = $("#isList").val(); 		 //是否显示
		var bdefault = $("#bdefault").val();	 //字典编码
		var isBT = $("#isBT").val();			 //是否必填
		var isSS = $("#isSS").val();			 //是否搜索项
		var svalue = $("#svalue").val();         //搜索条件
		...
		bdefault = bdefault == '' ? '无':bdefault;
		var fields = dname + ',fh,' + dtype + ',fh,' + dbz + ',fh,' + isQian + ',fh,' + ddefault + ',fh,' + isBox + ',fh,' + bdefault + ',fh,' + isList + ',fh,' + isBT + ',fh,' + isSS + ',fh,' + svalue;
		//属性名,类型,备注,是否前台录入,默认值,是否选项,字典编码,是否显示,是否必填,是否搜索项,搜索条件
		if(msgIndex == ''){
			arrayField(fields);
		}else{
	...
	function dialog_open(){
		...
		$("#isList").val("是");
		$("#isBT").val("是");
		$("#form-field-radiot1").attr("checked",true);
		$("#form-field-radioq1").attr("checked",true);
		$("#form-field-radiob2").attr("checked",true);
		$("#bdefault").attr("disabled", true);
		$("#bdefault").val("");
		$("#form-field-radiol1").attr("checked",true);
		$("#form-field-radiobt1").attr("checked",true);
		$("#dialog-add").css("display","block");
		$("#ddefault").attr("disabled",true);
		$("#sdefault").val("like");
		$("#sdefault").attr("disabled", true);
		$("#form-field-radioss2").attr("checked", true);
	}
	//打开编辑属性(修改)
	function editField(value,msgIndex){
		...
		$("#bdefault").val(efieldarray[6]);
		$("#msgIndex").val(msgIndex);
		if(efieldarray[1] == 'String'){
			$("#form-field-radiot1").attr("checked",true);
			$("#dtype").val('String');
		}else if(efieldarray[1] == 'Integer'){
			$("#form-field-radiot2").attr("checked",true);
			$("#dtype").val('Integer');
		}else if(efieldarray[1] == 'Date'){
			$("#form-field-radiot3").attr("checked",true);
			$("#dtype").val('Date');
		}
		else if(efieldarray[1] == 'Datetime'){
			$("#form-field-radiot4").attr("checked",true);
			$("#dtype").val('Datetime');
		}
		else{
			$("#form-field-radiot5").attr("checked",true);
			$("#dtype").val('Double');
		}
		if(efieldarray[3] == '是'){
			$("#form-field-radioq1").attr("checked",true);
			...
			$("#form-field-radioq2").attr("checked",true);
			$("#isQian").val('否');
		}
		if(efieldarray[5] == '是'){
			$("#form-field-radiob1").attr("checked",true);
			$("#isBox").val('是');
		}
		else{
			$("#form-field-radiob2").attr("checked",true);
		...
		if(efieldarray[7] == '是'){
			...
			$("#form-field-radiol2").attr("checked",true);
			$("#isList").val('否');
		}
		if(efieldarray[8] == '是'){
			$("#form-filed-radiobt1").attr("checked", true);
			$("#isBT").val('是');
		}
		else{
			$("#form-filed-radiobt2").attr("checked", true);
			$("#isBT").val('否');
		}
		if(efieldarray[9] == '是'){
			$("#form-field-radioss1").attr("checked", true);
			$("#isSS").val("是");
		}
		else{
			$("#form-field-radioss2").attr("checked", true);
		...
		if(typeof(efieldarray[10]) != "undefined" && efieldarray[10] != "无")
			$("#sdefault").val(efieldarray[10]);
		$("#dialog-add").css("display","block");
	}
	//关闭编辑属性
	function cancel_pl(){
	...
	//赋值搜索条件
	function sdefaultChange(){
		$("#svalue").val($("#sdefault").val());
	}
	...
	function appendC(value){
		...
		$("#fields").append(
			...
			'<td class="center">'+fieldarray[9]+'<input type="hidden" name="field9'+index+'" value="'+fieldarray[9]+'"></td>'+
